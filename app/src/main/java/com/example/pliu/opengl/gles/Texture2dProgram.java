...
package com.example.pliu.opengl.gles;
...
public class Texture2dProgram {
    ...
    public enum ProgramType {
        TEXTURE_2D, TEXTURE_EXT, TEXTURE_EXT_BW, TEXTURE_EXT_FILT, TEXTURE_EXT_TRANSPARENT
    }
    ...
    private static final String FRAGMENT_SHADER_EXT_TRANSPARENT =
            "#extension GL_OES_EGL_image_external : require\n"+
                    "precision mediump float;\n"+
                    "varying vec2 vTextureCoord;\n"+
                    "uniform samplerExternalOES sTexture;\n"+
                    ...
                    "\n"+
                    " if((1.0 - tc.r) < 0.01 && (1.0 - tc.g) < 0.01 && (1.0 - tc.b) < 0.01) {\n"+
                    " gl_FragColor = bg;\n"+
                    " } else {\n"+
                    " gl_FragColor = tc;\n"+
                    " }\n"+
    ...
    private int mTextureLoc, mBgTextureLoc;
    ...
    public Texture2dProgram(ProgramType programType) {
        ...
        switch (programType) {
            ...
            case TEXTURE_EXT_TRANSPARENT:
                mTextureTarget = GLES11Ext.GL_TEXTURE_EXTERNAL_OES;
                mProgramHandle = GlUtil.createProgram(VERTEX_SHADER, FRAGMENT_SHADER_EXT_TRANSPARENT);
                break;
        ...
        mTextureLoc = GLES20.glGetUniformLocation(mProgramHandle, "sTexture");
        mBgTextureLoc = GLES20.glGetUniformLocation(mProgramHandle, "sFilterTexture");
    }
    ...
    public void draw(float[] mvpMatrix, FloatBuffer vertexBuffer, int firstVertex,
                     int vertexCount, int coordsPerVertex, int vertexStride,
                     float[] texMatrix, FloatBuffer texBuffer, int[] textureIds, int texStride) {
        GlUtil.checkGlError("draw start");
        ...
        for(int i = 0; i < textureIds.length; i++) {
            switch (i) {
                case 0: {
                    ...
                    GLES20.glUniform1i(mBgTextureLoc, 0);
                    continue;
                }
                case 1: {
                    GLES20.glActiveTexture(GLES20.GL_TEXTURE1);
                    GLES20.glBindTexture(mTextureTarget, textureIds[1]);
                    GLES20.glUniform1i(mTextureLoc, 1);
                    continue;
                }
                default: {
                    throw new RuntimeException("texture length is overflow, max:3, current:" + i);
                }
